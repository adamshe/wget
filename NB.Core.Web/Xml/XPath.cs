...
namespace NB.Core.Web.Xml
{
    ...
    public class XPath
    {
        ...
        public XParseElement GetElement(XContainer container)
        {
            XParseElement[] nodes = this.GetElements(container, true);
            if (nodes.Length == 0) { return null; }
            else { return nodes[0]; }
        }
        public XParseElement[] GetElements(XContainer container)
        {
            ...
            if (this.IsRootPath)
            {
                if (container is XParseElement && ((XParseElement)container).Name.LocalName == mChild.mName)
                {
                    ...
                    else if (mChild.mExtensionType == TokenExtensionType.AttributeID)
                    {
                        ...
                        if (att != null && this.StringValuesEquals(mChild.mAttributeValue, att.Value)) { matchNodes.Add((XParseElement)container); }
                    }
                }
        ...
        public static XParseElement GetElement(string xpath, XContainer conatiner, bool regexAttributes = false)
        {
            return (new XPath(xpath) { RegexAttributeValue = regexAttributes }).GetElement(conatiner);
        }
        public static XParseElement[] GetElements(string xpath, XContainer conatiner, bool regexAttributes = false)
        {

...
namespace NB.Core.Web.Xml
{
    ...
    internal sealed class XmlParser
    {
        ...
        public static XParseDocument Parse(string text) { return new XmlParser().ParseDocument(text); }
        ...
        private XParseName ParseName(StringCounter sc)
        {
            ...
            if (n != string.Empty) name = XParseName.Get(n.Replace("h1", "hx").Replace("h2", "hx"), ns);
            sc.TrimStart();
            return name;
        }
        private XParseAttribute ParseAttribute(StringCounter sc)
        {
            sc.TrimStart();
            XParseAttribute att = null;
            XParseName name = null;
            ...
            if (name != null && sc.StartValue == '=')
            {
                ...
                att = new XParseAttribute(name, XmlParser.DecodeXml(val));
            }
                    ...
                    for (int i = sc.Index; i < sc.Value.Length; i++)
                    {
                        if (!isComment && !isDeclaration)
                        {
                            ...
                            {                                                     
                                ...
                                {
                                    XParseAttribute att = ParseAttribute(sc.NewIndex(i));
                                    i = sc.Index - 1;
                                    if (att != null)
                                    {
                                        node.Add(att);
                        ...
                        else
                        {
                            ...
                            else if (isDeclaration)
                            {
                                ...
                                {
                                    if (parent != null && parent is XParseDocument) ((XParseDocument)parent).Declaration = new XDeclaration(declVer, declEnc, declSta);
                                    sc.Index = i + 2;
                                    break;
                                }
                                else if (IsValidTagChar(sc.Value[i]))
                    ...
                    if (node != null && !elementAtEnd)
                    {
                        ...
                        for (int i = sc.Index; i < sc.Value.Length; i++)
                        {
                            if (sc.Value[i] == '<')
                            {
                                ...
                                {
                                    //End Tag
                                    XParseName endName = ParseName(sc.NewIndex(i + 2));
                                    if (endName != null)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            {
                                                //Other End Name
                                                XContainer par = ((parent is XParseElement && XmlParser.CompareXName(((XParseElement)parent).Name, endName)) ? parent : (XContainer)this.FindParent(parent, endName));
                                                if (par != null)
                                                {
                                                    if (name.LocalName != "form")
                                                    {
                                                        XParseElement[] enm = MyHelper.EnumToArray(node.Elements());
                                                        if (enm.Length > 0)
                                                        {
                                                            foreach (XParseElement elem in enm)
                                                            {
